...
class MBConvBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if in_channels != hidden_dim:
            layers += [ConvBNReLU(in_channels, out_channels, kernel_size=1)]
...
def _round_filter(filters, width_mult):
    if width_mult == 1.0:
        return filters
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, width_mult=1.0, depth_mult=1.0, dropout_rate=0.2, num_classes=1000):
        ...
        out_channels = _round_filter(32, width_mult)
        features = [ConvBNReLU(3, out_channels, kernel_size=3, stride=2)]
        
        in_channels = out_channels
        for t, c, n, s, k in settings:
            out_channels = _round_filter(c, width_mult)
            repeats = _round_repeats(n, depth_mult)
            for i in range(repeats):
                stride = s if i == 0 else 1
                features += [MBConvBlock(in_channels, out_channels, expand_ratio=1, stride=stride, kernel_size=k)]
                in_channels = out_channels
            
        last_channels = _round_filter(1280, width_mult)
        features += [ConvBNReLU(in_channels, last_channels, kernel_size=1)]
...
def _efficientnet(arch, pretrained, progress, **kwargs):
    ...
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls[arch], progress=process)
